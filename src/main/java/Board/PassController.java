...
public class PassController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		req.getRequestDispatcher("/Main/Pass.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		boolean confirmed = dao.confirmPassword(num);
		dao.close();
		...
		if(confirmed) {
			if(mode.equals("edit")) {
				...
				수정 처리가 되지않도록 처리할것이다.
				 */
				resp.sendRedirect("../gallclimb/edit.do?num="+num);
			}
				...
				if(result==1) {
					...
					FileUtil.deleteFile(req, "/UploadsDirectory", saveFileName);
				}
				//삭제가 완료되면 리스트로 이동한다.
				JSFunction.alertLocation(resp, "삭제되었습니다.", "../gallclimb/List.do");
			}
		}
		else {
